...
/**
 * PermissionGroupsPage class handles Permission Groups functionality
 * Part of Roles & Permissions module
 */
class PermissionGroupsPage extends BasePage {
  constructor(page) {
    super(page);

    // Page URL
    this.url = '/administration/permission-groups';
    ...
    this.selectors = {
      ...
      permissionsCheckbox: 'input[type="checkbox"]',
   ...
   */
  async navigateToPermissionGroups(baseURL = 'https://10.10.10.10:700') {
    await this.logAction('Navigation', `Navigating to Permission Groups page`);
    await this.goto(`${baseURL}${this.url}`);
    await this.waitForNetworkIdle();
    await this.logAction('Navigation', 'Permission Groups page loaded');
  }
  ...
  async capturePageState(name) {
    await this.takeScreenshot(`permission-groups-${name}`);
  }
}

module.exports = PermissionGroupsPage;
