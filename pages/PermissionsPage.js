...
/**
 * PermissionsPage class handles Permissions functionality
...
class PermissionsPage extends BasePage {
  constructor(page) {
    ...
    // Page URL
    this.url = '/administration/permissions';
    ...
    this.selectors = {
      ...
      moduleSelect:
        'mat-select[formControlName="module"], select[name="module"]',
   ...
   */
  async navigateToPermissions(baseURL = 'https://10.10.10.10:700') {
    await this.logAction('Navigation', `Navigating to Permissions page`);
    await this.goto(`${baseURL}${this.url}`);
    await this.waitForNetworkIdle();
    await this.logAction('Navigation', 'Permissions page loaded');
  }
  ...
  async capturePageState(name) {
    await this.takeScreenshot(`permissions-${name}`);
  }
}
